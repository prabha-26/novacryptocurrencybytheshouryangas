...
const App: React.FC = () => {
  ...
  // Fetch Real Market Data based on selected currency
  useEffect(() => {
    ...
    loadMarketData();
    // Refresh every 60 seconds (Full Sync)
  ...
  useEffect(() => {
    ...
    const liveInterval = setInterval(() => {
        setCoins(prevCoins => prevCoins.map(coin => {
            // Random jitter between -0.2% and +0.2%
            const change = 1 + (Math.random() * 0.004 - 0.002);
            return {
                ...coin,
                current_price: coin.current_price * change
            };
        }));
    }, 3000);
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        {marketLoading ? `Connecting to ${currency.toUpperCase()} market data...` : `Real-time prices via CoinGecko in ${currency.toUpperCase()}`}
                    </p>
                </div>
                <button 
